package com.lk.service.system.menu.impl;
...
public class MenuService implements MenuManager{
	...
	public List<Menu> listAllMenuQx(String MENU_ID) throws Exception {
		List<Menu> menuList = this.listSubMenuByParentId(MENU_ID); //通过ID获取其子一级菜单
		for(Menu menu : menuList){
	...
	public List<Menu> listMenu(PageData pd) throws Exception {
		return (List<Menu>) dao.findForList("MenuMapper.listMenu", pd);
	...
	public PageData findByName(PageData pdMessage) throws Exception{
		return (PageData) dao.findForObject("MenuMapper.findByName", pdMessage);
	}

	@Override
	public PageData findByUrlAndName(PageData pdMenu) throws Exception{
		return (PageData) dao.findForObject("MenuMapper.findByUrlAndName", pdMenu);
	}
