					...
					<div class="row">
						<div class="col-xs-12">
							...
							<table class="tablemargin">
									...
									<td>
										<div class="nav-search">
											<span class="input-icon">
												<input type="text" placeholder="请输入奖品名称" class="nav-search-input" id="nav-search-input" autocomplete="off" name="keywords" value="${pd.keywords }" placeholder="这里输入关键词"/>
								...
								<p style="color: red;float: right;position: relative;top: 3px;">注：奖品需要先禁用才能删除</p>
							</table>
							<!-- 检索  -->
							...
							<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
								<thead>
									<tr>
										...
										<%--<th class="left">奖品名称</th>--%>
                                        <th class="left">网吧名称</th>
										<%--<th class="left">是否有奖</th>--%>
										<%--<th class="left">中奖概率</th>--%>
										<%--<th class="left">奖品有效期(天)</th>--%>
                                        <th class="left">奖品数量</th>
										...
										<c:forEach items="${varList}" var="var" varStatus="vs">
											<tr>
												<td class='center' style="vertical-align: middle">
													<label class="pos-rel">
														<input type='checkbox' name='ids' value="${var.LOTTERY_ID}" class="ace" />
														<span class="lbl"></span>
													</label>
												</td>
												...
												<td class='left' style="vertical-align: middle">${var.LOTTERY_NAME}</td>
												  <%--<td class='left' style="vertical-align: middle">--%>
												    <%--<c:if test="${var.STATE =='0'}">有</c:if>--%>
												    <%--<c:if test="${var.STATE =='1'}">没有</c:if>--%>
											     <%--</td>--%>
												<%--<td class='left' style="vertical-align: middle">${var.LOTTERY_NUMBER}</td>--%>
												<td class='left' style="vertical-align: middle">${var.EXPIRY_DATE}</td>
												<td class="center" style="vertical-align: middle">
													...
													<div class="hidden-sm hidden-xs btn-group">
														<c:if test="${var.USING_STATE == '1' }">
															<a class="btn btn-sm btn-default" title="禁用" onclick="usingState('${var.LOTTERY_ID}','1');">
																<i class="layui-icon" style="padding-right: 4px">&#x1006;</i>禁用
															</a>
														</c:if>
														<c:if test="${var.USING_STATE == '2' }">
															<a class="btn btn-sm btn-warning" title="启用" onclick="usingState('${var.LOTTERY_ID}','2');">
																<i class="layui-icon" style="padding-right: 4px">&#xe605;</i>启用
															</a>
														</c:if>
														<c:if test="${QX.edit == 1}">
															<a class="btn btn-sm btn-success" title="编辑" onclick="edit('${var.LOTTERY_ID}');">
																<i class="layui-icon" style="padding-right: 4px">&#xe642;</i>编辑
															</a>
														</c:if>
														<c:if test="${QX.del == 1 }">
															<c:if test="${var.USING_STATE == '2' }">
																<a class="btn btn-sm btn-danger" onclick="del('${var.LOTTERY_ID}');">
													...
													<div class="hidden-md hidden-lg">
														<div class="inline pos-rel lk-ul-hoverShow">
															...
															<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
																<c:if test="${var.USING_STATE == '1' }">
																<li>
																	<a style="cursor: pointer;" onclick="usingState('${var.LOTTERY_ID}','1');" class="tooltip-success" data-rel="tooltip" title="禁用">
																...
																<c:if test="${var.USING_STATE == '2' }">
																<li>
																	<a style="cursor: pointer;"onclick="usingState('${var.LOTTERY_ID}','2');" class="tooltip-success" data-rel="tooltip" title="启用">
																...
																<c:if test="${QX.edit == 1 }">
																<li>
																	<a style="cursor: pointer;" onclick="edit('${var.LOTTERY_ID}');" class="tooltip-success" data-rel="tooltip" title="编辑">
																...
																<c:if test="${QX.del == 1 }">
																<li>
																	<a style="cursor: pointer;" onclick="del('${var.LOTTERY_ID}');" class="tooltip-success" data-rel="tooltip" title="删除">
	...
	<script type="text/javascript">
		...
		function usingState(Id,type){
			...
			layer.confirm(message, {
            	...
            	$.ajax({
					type: "POST",
					url: '<%=basePath%>lottery/usingState.do',
    				data: {LOTTERY_ID:Id},
					dataType:'json',
					cache: false,
		...
		function add(ID){
			layer.open({
               	...
               	fixed: false, //不固定
 				maxmin: true,
               	content:[ '<%=basePath%>lottery/goAdd.do' ],
           	});
		}
		...
		function del(Id){
			layer.confirm('确定要删除吗?', {
           		...
           		$.ajax({
					type: "POST",
					url: '<%=basePath%>lottery/delete.do',
    				data: {LOTTERY_ID:Id},
					dataType:'json',
					cache: false,
					success: function(data){
		...
		function edit(ID){
			layer.open({
               	...
               	fixed: false, //不固定
 					maxmin: true,
               	content:[ '<%=basePath%>lottery/goEdit.do?LOTTERY_ID='+ID ],
           	});
		}
			...
			layer.confirm('确定要删除选中的数据吗?', {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							$.ajax({
								type: "POST",
								url: '<%=basePath%>lottery/deleteAll.do',
			    				data: {DATA_IDS:str},
								dataType:'json',
								cache: false,
								success: function(data){
