...
<script>
	...
	var url = "<%=basePath%>wxMatches/loadGameList.do?flag=myEnroll";
		...
		$.post(url,{'type':type}, function (res){
			...
			if(list.length >0){
				for (var i = 0; i < list.length; i++) {
					...
					htmls += '<div class="weic-gameList-box" onclick=go_gameDetails("'+data.id+'")>'+
				...
				htmls = '<div class="weic-noboby gamesList" style="height: 6.2rem;">'+
						...
						'<div class="weic-nobody-text">暂无报名赛事记录</div>'+
						'</div>'
			}
			$("#gameList").html(htmls);
