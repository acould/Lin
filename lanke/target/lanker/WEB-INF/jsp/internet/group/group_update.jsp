  ...
  <div class="borPadding">
        <form class="layui-form layui-form-pane" name="From" id="From" action="<%=basePath%>group/${msg }.do">
					...
					<c:choose>
						<c:when test="${not empty storeList}">
							<c:forEach items="${storeList}" var="var" varStatus="vs">
									<!--<input type="checkbox" name="STORE_ID" id="STORE_ID" value="${var.STORE_ID }" onclick="checkAll()" title="${var.STORE_NAME }"  lay-skin="primary" />-->
									<c:if test="${sb != null }">
										<input type="checkbox" name="STORE_ID" id="STORE_ID" value="${var.STORE_ID }"
											<c:if test="${sb.indexOf(var.STORE_ID)>-1 }">checked="checked" </c:if>
											<c:if test="${pd.STATE == 2 }">disabled="disabled"</c:if> title="${var.STORE_NAME }" lay-filter="store" lay-skin="primary" />
									</c:if>
									<c:if test="${sb == null }">
										<input type="checkbox" checked="checked" name="STORE_ID" id="STORE_ID" value="${var.STORE_ID }" lay-filter="store" title="${var.STORE_NAME }" 
										<c:if test="${pd.STATE == 2 }">disabled="disabled" </c:if> lay-skin="primary"/>
									</c:if>
							</c:forEach>
						</c:when>
					</c:choose>
						<input type="checkbox" name="allChecked" id="allChecked" onclick="DoCheck()" <c:if test="${pd.STATE == 2 }">disabled="disabled" </c:if> lay-skin="primary" title="全选" lay-filter="tufure">
                </div>
            </div>
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-block" >
                    <input type="text" class="layui-input" name="BEGIN_TIME" id="BEGIN_TIME" value="${pd.BEGIN_TIME}" placeholder="yyyy-MM-dd HH:mm:ss" <c:if test="${pd.STATE == 2 }">disabled="disabled"</c:if> >
                </div>
            </div>
            <div class="layui-form-item">
                <label class="layui-form-label" style="margin:0">报名截止：</label>
                <div class="layui-input-block" >
                    <input type="text" class="layui-input" name="BM_TIME" id="BM_TIME" value="${pd.BM_TIME}" placeholder="yyyy-MM-dd HH:mm:ss" <c:if test="${pd.STATE == 2 }">disabled="disabled"</c:if> >
                </div>
            </div>
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-block">
                    <input type="number" name="TEAM_NUMBER" id="TEAM_NUMBER" value="${pd.TEAM_NUMBER}" autocomplete="off" placeholder="请输入战队数量" class="layui-input" <c:if test="${pd.STATE == 2 }">readonly=true</c:if> >
                </div>
            </div>
            <div class="layui-form-item">
                <label class="layui-form-label" style="margin:0">每队人数：</label>
                <div class="layui-input-inline" >
                    <input type="number" name="MIN_NUMBER" id="MIN_NUMBER" value="${pd.MIN_NUMBER}" autocomplete="off" placeholder="请输入最低人数" class="layui-input" <c:if test="${pd.STATE == 2 }">readonly=true</c:if>>
                </div>
                <div class="layui-input-inline" >
                    <input type="number" name="MAX_NUMBER" id="MAX_NUMBER" value="${pd.MAX_NUMBER}" autocomplete="off" placeholder="请输入最高人数" class="layui-input" <c:if test="${pd.STATE == 2 }">readonly=true</c:if> >
                </div>
            </div>
