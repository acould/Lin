...
//更新消息通知数量
function getMessageNum(basePath){
	$.ajax({
		type: "POST",
		url: basePath+'newerGuide/getMessageNum.do',
		data: {},
		dataType:'json',
		cache: false,
		success: function(data){
			if(data.number > 0){
				$("#aId").html('消息通知<span class="layui-badge" id="msgNum">'+data.number+'</span>');
			}else{
				$("#aId").html('消息通知');
			}
		}
...
function editUserH(){
	...
	layer.open({
		...
		fixed: false, //不固定
		maxmin: true,
		content:[ 'user/goEditU.do?USER_ID='+USER_ID+'&fx=head' ],
	});
