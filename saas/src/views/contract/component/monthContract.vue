<template>
    <div class="month-contract">
        <i-tabs v-model="activeName">
            <i-tab-pane label="合同信息" name="first">
                <div class="info-box">
            ...
            <i-tab-pane label="租期账单" name="second">
                <div>
                    <div>
...
export default {
    ...
    methods:{
        ...
        getDetail(id){
            ...
            contractService.getMonthContractDetail(id).then(res => {
                ...
                if (res.code === 0) {
					...
					this.form.signDate = new Date(this.form.signDate);
                    this.state = 'update'; 
                }
            });
		},
		...
		createBill(){
			// this.$emit('next');;return ;
			this.$refs['form'].validate(valid => {
                if (valid) {
					if( typeof this.form.bills !=='undefined' && this.form.bills.length!=0 ){
						this.openReceiptList();
						this.$emit('next');
						return false;
					}
					if(this.form.otherFees.length==1){
						if(this.form.otherFees[0].feeType=='' || this.form.otherFees[0].paymentType=='' || this.form.otherFees[0].money=='' || this.form.otherFees[0].feeId=='' ){
							this.form.otherFees=[];
						}
					}
