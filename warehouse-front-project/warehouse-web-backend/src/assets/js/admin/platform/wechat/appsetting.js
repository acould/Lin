    ...
    var module = {
        ...
        event: function() {
            ...
            this.$wrap.find('.J_Delete').click(function() {
             ...
             // 发布菜单
            this.$wrap.find('.J_PublishMenu').click(function() {
                self.onPublishMenu($(this));
            });
        },
        ...
        onSave: function($elem) {
            ...
            this.ajax('/wechat/appsetting/save', data, function(data) {
                ...
                window.location.href = "/wechat/appsetting/list.html";
            });
        },
        ...
        onDelete: function($elem) {
         ...
         // 发布菜单
        onPublishMenu: function($elem) {
            var name = $elem.closest('tr').find('.gzhName').text(),
                data = {
                    id: $elem.data('id')
                };

            if (confirm('是否发布公众号名称为 “' + name + '”公众号菜单 ?')) {
                this.ajax('/wechat/appsetting/publishMenu', data, function(resultDes) {
                    alert(resultDes);
                    //window.location.reload();
                });
            }
