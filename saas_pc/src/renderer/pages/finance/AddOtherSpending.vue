    ...
    <div id="main-content-addspending">
      <div class="form">
        <el-form :model="formParams" :rules="formRules" ref="releaseForm" :label-width="labelWidth" class="mission-form">
          ...
          <el-form-item label="支出内容" prop="remark" class="inline-item inline-item-iconfont block">
            <div class="inp-wrap">
              <el-input v-model.trim="formParams.remark" placeholder="请输入支出内容"></el-input>
            </div>
          </el-form-item>
          <el-form-item label="价格明细" prop="other_money" class="inline-item inline-item-iconfont block">
            <div class="inp-wrap">
              <el-input v-model.trim="formParams.other_money" placeholder="请输入支出价格"></el-input>
            </div>
          </el-form-item>
        </el-form>
      </div>
...
export default {
  name: 'AddSpending',
  data () {
    var validateOtherMoney = (rule, value, callback) => {
      let reg = /^[1-9]\d*$/g
      if (!value) {
        callback(new Error('支出价格不能为空'))
      } else if (!reg.test(value)) {
        callback(new Error('请输入正确支出价格'))
      } else {
        callback()
      }
    }
    return {
      ...
      formRules: {
        ...
        other_money: { required: true, validator: validateOtherMoney, trigger: ['blur', 'change'] },
        remark: { required: true, message: '支出内容不能为空', trigger: ['blur', 'change'] },
      },
      images: [],
      imgLen: 0,
