 ...
 */
@Controller
@RequestMapping("/warehouse/storagearea")
public class StorageAreaController extends BasicController {

	@Autowired
	private StorageAreaFacade storageAreaFacade;
	...
	public String list(Model model, StorageAreaVo dataVo) throws BizRuleException {
		...
		if(dataVo==null || (dataVo!=null && dataVo.getParentId()==null || dataVo.getParentId().longValue()==0)) {
			searchMap.put("parentId",0);//第一级
		}else {
			searchMap.put("parentId",dataVo.getParentId());//其他货区级别
		}
		ManagerVo sysUser = this.getSysUser();
		SearchResult<StorageAreaVo> searchResult = storageAreaFacade.getListTotalByWhere(sysUser.getMerchantId(),searchMap,vo.getStart(),vo.getSize());

		vo.setList(searchResult.getList());
		vo.setCount(searchResult.getTotal());
		SearchUtil.putToModel(model, vo);
		
		return "/warehouse/storagearea/list";
	...
	public String edit(Model model,Long id) throws BizRuleException{
		StorageAreaVo dataVo=new StorageAreaVo();
		if(id!=null&&id.intValue()!=0){//修改
			ManagerVo sysUser = this.getSysUser();
			dataVo = storageAreaFacade.getByPrimaryKey(sysUser.getMerchantId(),id);									
		}
		model.addAttribute("dataVo",dataVo);				
		return "/warehouse/storagearea/edit";
	}
	...
	@ResponseBody
	@RequestMapping("/save")
	public Model save(HttpServletRequest reqeust, StorageAreaVo dataVo) throws BizRuleException {	
		...
		if(dataVo.getId() == null){
			dataid = storageAreaFacade.insertStorageAreaVo(loginUser.getMerchantId(),dataVo);
		}else{
			dataid = storageAreaFacade.updateStorageArea(loginUser.getMerchantId(),dataVo);
		}				
	...
	public Model delete(Model model, HttpServletRequest reqeust, Long id) throws BizRuleException {
		...
		ManagerVo sysUser = this.getSysUser();
		storageAreaFacade.deleteBy(sysUser.getMerchantId(),id);
