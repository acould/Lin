 ...
 */
@Controller
@RequestMapping("/product/storeout")
public class ProductStoreOutController extends BasicController {

	@Autowired
	private StoreOutFacade storeOutFacade;
	...
	public String list(Model model, StoreOutVo dataVo) throws BizRuleException {
		...
		SearchResult<StoreOutVo> searchResult = storeOutFacade.getListTotalByWhere(sysUser.getMerchantId(),searchMap,vo.getStart(),vo.getSize());

		vo.setList(searchResult.getList());
		vo.setCount(searchResult.getTotal());
		SearchUtil.putToModel(model, vo);
		
		return "/product/storeout/list";
	...
	public String edit(Model model,Long id) throws BizRuleException{
		StoreOutVo dataVo=new StoreOutVo();
		if(id!=null&&id.intValue()!=0){//修改
			ManagerVo sysUser = this.getSysUser();
			dataVo = storeOutFacade.getByPrimaryKey(sysUser.getMerchantId(),id);									
		}
		model.addAttribute("dataVo",dataVo);				
		return "/product/storeout/edit";
	}
	...
	@ResponseBody
	@RequestMapping("/save")
	public Model save(HttpServletRequest reqeust, StoreOutVo dataVo) throws BizRuleException {	
		...
		if(dataVo.getId() == null){
			dataid = storeOutFacade.insertStoreOutVo(loginUser.getMerchantId(),dataVo);
		}else{
			dataid = storeOutFacade.updateStoreOut(loginUser.getMerchantId(),dataVo);
		}				
	...
	public Model delete(Model model, HttpServletRequest reqeust, Long id) throws BizRuleException {
		...
		ManagerVo sysUser = this.getSysUser();
		storeOutFacade.deleteBy(sysUser.getMerchantId(),id);
