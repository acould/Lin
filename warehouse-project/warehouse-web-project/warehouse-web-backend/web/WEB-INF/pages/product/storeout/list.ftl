...
<li><a href="/product/storeout/list.html">出库单管理</a></li>
				...
				<div class="col-xs-12">
					...
					<div class="page-header">
						<table style="width:98%;">
							<tr>
								<td><a href="/product/storeout/edit.html" class="btn btn-primary">添 加</a>
								</td>
								<td style="text-align:right" >
									 <form action="/product/storeout/list.html" method="get" class="form_search">
							        	<label>货币名称：</label>
								        <input type="text" name="currencyName" value="${currencyName!}" placeholder="请输入货币名称..." class="searchField base-input">
					...
					<div id="wrap">
							   ...
							   <#if (bizObj.list?exists && bizObj.list?size>0)>
									<#list bizObj.list as r>
										<tr>
											...
											<td>
												<a href="#">${r.currencyName!}</a>
											</td>
											<td>${r.code!}</td>
											<td class="hidden-480">${r.baseCurrencyCode!} </td>
											...
											<td>
												<div class="hidden-sm hidden-xs action-buttons">
													<a class="green" href="/product/storeout/edit.html?id=${r.id}">
						...
						<tfoot>
							<tr>
								<td colspan="100%">
									<#if (name)??><#assign searchParam="&currencyName=${currencyName!}"><#elseif (url)??><#assign searchParam="&url=${url?url}"></#if>
									<@model.showPage url=vm.getUrlByRemoveKey(thisUrl,['start','size']) p=bizObj.page parEnd="cmd=search&currencyName=${currencyName!}${searchParam!}" />
								</td>
							</tr>
						</tfoot>
					</div>
...
<script src="${model.static_domain}/js/admin/product/storeout/list.min.js?v=${model.static_version}"></script>
</#assign>
