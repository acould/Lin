								...
								<td style="text-align:right" >
									 ...
									 <#--  
									 	<#if (warehouseClassDTOList?exists && warehouseClassDTOList?size>0) >
											<select name="warehouseClassId">
												...
												<#list warehouseClassDTOList as r> -->
													<#-- <option value="${r.id!}" <#if (warehouseClassId?? && (r.id == warehouseClassId) )>selected</#if>>${r.name!}</option>   -->
											<#-- 	
										...
										<select name="warehouseType">
										 ...
										 -->	
										<input type="text" name="code" value="${code!}" placeholder="请输入仓库编码..." class="searchField base-input">	
								        <input type="text" name="name" value="${name!}" placeholder="请输入仓库名称..." class="searchField base-input">
