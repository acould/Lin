...
import star.warehouse.mapper.StorageAreaMapper;
import star.warehouse.po.StorageArea;

@Service
public class StorageAreaService {
	private static final Logger logger = LoggerFactory.getLogger(StorageAreaService.class);
	@Resource
	...
	public Long insertStorageAreaVo(Long merchantId,StorageAreaVo vo) throws BizRuleException {
		...
		StorageArea po =vo.copyTo(StorageArea.class);
		storageAreaMapper.insertStorageArea(po);
		return po.getId();
	}
	...
	public StorageAreaVo getByPrimaryKey(Long merchantId,Long id) throws BizRuleException {
		...
		StorageArea po = storageAreaMapper.getByPrimaryKey(id);
		...
		return po.copyTo(StorageAreaVo.class);
	}
	...
	public Long updateStorageArea(Long merchantId,StorageAreaVo vo) throws BizRuleException {
		...
		StorageArea po =vo.copyTo(StorageArea.class);
		storageAreaMapper.updateStorageArea(po);
		return vo.getId();
	}
	 ...
	 * @haoxz11MyBatis
	 */
	public List<StorageAreaVo> getListByWhere(Long merchantId,HashMap<String, Object> searchMap,int start,int size) throws BizRuleException{
		...
		List<StorageArea> retlist= storageAreaMapper.getListByWhere(searchMap,new RowBounds(start,size));
		if(CollectionUtils.isEmpty(retlist)) {
			return Collections.emptyList();
		}
		return BaseVo.copyListTo(retlist, StorageAreaVo.class);
	}
	...
	public List<StorageAreaVo> getListByWhere(Long merchantId,HashMap<String, Object> searchMap) throws BizRuleException{
		...
		searchMap.put("merchantId", merchantId);
		List<StorageArea> retlist= storageAreaMapper.getListByWhere(searchMap);
		if(CollectionUtils.isEmpty(retlist)) {
			return Collections.emptyList();
		}
		return BaseVo.copyListTo(retlist, StorageAreaVo.class);
	...
	public int getCountByWhere(Long merchantId,HashMap<String, Object> searchMap) throws BizRuleException {
		...
		searchMap.put("merchantId", merchantId);
		return storageAreaMapper.getCountByWhere(searchMap);
	...
	public List<StorageAreaDTO> getCacheStorageAreaDTOList(Long merchantId) throws BizRuleException{
		if(merchantId==null || merchantId.longValue()<0) {
			logger.error("getCacheStorageAreaDTOList merchantId is null");
		...
		return new CacheKeyLock(cachesKeyService, SysCacheTimeDMO.CACHETIMEOUT_30M) {//30分钟
			...
			protected Object doGetList(BaseCacheEnum type, String key) {
				...
				return BaseVo.copyListTo(retlist, StorageAreaDTO.class);
			}
		}.getCache(CacheWarehouseEnum.STORAGEAREADTO_LIST,String.valueOf(merchantId), RedisCacheWaitTimeDMO.REDIS_TIMEOUT_1S);
