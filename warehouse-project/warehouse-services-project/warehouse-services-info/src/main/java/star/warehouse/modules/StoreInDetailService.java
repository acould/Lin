...
import star.facade.warehouse.vo.StoreInDetailVo;
import star.modules.cache.CachesKeyService;
import star.vo.BaseVo;
import star.warehouse.mapper.StoreInDetailMapper;
import star.warehouse.po.StoreInDetail;

@Service
public class StoreInDetailService {
	private static final Logger logger = LoggerFactory.getLogger(StoreInDetailService.class);
	@Resource
	 ...
	 */
	public Long insertStoreInDetailVo(Long merchantId,StoreInDetailVo vo) throws BizRuleException {
		...
		StoreInDetail po =vo.copyTo(StoreInDetail.class);
		storeInDetailMapper.insertStoreInDetail(po);
		return po.getId();
	}
	 ...
	 */
	public StoreInDetailVo getByPrimaryKey(Long merchantId,Long id) throws BizRuleException {
		if(id==null || id<0) {
			logger.error("getByPrimaryKey id is null");
			throw new BizRuleException("参数id不能为空");
		}
		StoreInDetail po = storeInDetailMapper.getByPrimaryKey(id);
		...
		return po.copyTo(StoreInDetailVo.class);
	}
	 ...
	 */
	public Long updateStoreInDetail(Long merchantId,StoreInDetailVo vo) throws BizRuleException {
		...
		StoreInDetail po =vo.copyTo(StoreInDetail.class);
		storeInDetailMapper.updateStoreInDetail(po);
		return vo.getId();
	}
	 ...
	 * @haoxz11MyBatis
	 */
	public List<StoreInDetailVo> getListByWhere(Long merchantId,HashMap<String, Object> searchMap,int start,int size) throws BizRuleException{
		...
		List<StoreInDetail> retlist= storeInDetailMapper.getListByWhere(searchMap,new RowBounds(start,size));
		if(CollectionUtils.isEmpty(retlist)) {
			return Collections.emptyList();
		}
		return BaseVo.copyListTo(retlist, StoreInDetailVo.class);
	}
	 ...
	 */
	public List<StoreInDetailVo> getListByWhere(Long merchantId,HashMap<String, Object> searchMap) throws BizRuleException{
		...
		searchMap.put("merchantId", merchantId);
		List<StoreInDetail> retlist= storeInDetailMapper.getListByWhere(searchMap);
		if(CollectionUtils.isEmpty(retlist)) {
			return Collections.emptyList();
		}
		return BaseVo.copyListTo(retlist, StoreInDetailVo.class);
	...
	public int getCountByWhere(Long merchantId,HashMap<String, Object> searchMap) throws BizRuleException {
		...
		searchMap.put("merchantId", merchantId);
		return storeInDetailMapper.getCountByWhere(searchMap);
