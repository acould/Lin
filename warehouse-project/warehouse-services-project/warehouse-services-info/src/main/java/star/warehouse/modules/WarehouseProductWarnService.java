...
import star.warehouse.mapper.WarehouseProductWarnInfoMapper;
import star.warehouse.po.WarehouseProductWarnInfo;

@Service
public class WarehouseProductWarnService {
	private static final Logger logger = LoggerFactory.getLogger(WarehouseProductWarnService.class);
	@Resource
	private WarehouseProductWarnInfoMapper warehouseProductWarnInfoMapper;
	@Resource
	...
	public Long insertWarehouseProductWarnInfoVo(Long merchantId,WarehouseProductWarnInfoVo vo) throws BizRuleException {
		...
		WarehouseProductWarnInfo po =vo.copyTo(WarehouseProductWarnInfo.class);
		warehouseProductWarnInfoMapper.insertWarehouseProductWarnInfo(po);
		return po.getId();
	}
	...
	public WarehouseProductWarnInfoVo getByPrimaryKey(Long merchantId,Long id) throws BizRuleException {
		...
		WarehouseProductWarnInfo po = warehouseProductWarnInfoMapper.getByPrimaryKey(id);
		...
		return po.copyTo(WarehouseProductWarnInfoVo.class);
	}
	...
	public Long updateWarehouseProductWarnInfo(Long merchantId,WarehouseProductWarnInfoVo vo) throws BizRuleException {
		...
		WarehouseProductWarnInfo po =vo.copyTo(WarehouseProductWarnInfo.class);
		warehouseProductWarnInfoMapper.updateWarehouseProductWarnInfo(po);
		return vo.getId();
	}
	 ...
	 * @haoxz11MyBatis
	 */
	public List<WarehouseProductWarnInfoVo> getListByWhere(Long merchantId,HashMap<String, Object> searchMap,int start,int size) throws BizRuleException{
		...
		List<WarehouseProductWarnInfo> retlist= warehouseProductWarnInfoMapper.getListByWhere(searchMap,new RowBounds(start,size));
		if(CollectionUtils.isEmpty(retlist)) {
			return Collections.emptyList();
		}
		return BaseVo.copyListTo(retlist, WarehouseProductWarnInfoVo.class);
	}
	...
	public List<WarehouseProductWarnInfoVo> getListByWhere(Long merchantId,HashMap<String, Object> searchMap) throws BizRuleException{
		...
		searchMap.put("merchantId", merchantId);
		List<WarehouseProductWarnInfo> retlist= warehouseProductWarnInfoMapper.getListByWhere(searchMap);
		if(CollectionUtils.isEmpty(retlist)) {
			return Collections.emptyList();
		}
		return BaseVo.copyListTo(retlist, WarehouseProductWarnInfoVo.class);
	...
	public int getCountByWhere(Long merchantId,HashMap<String, Object> searchMap) throws BizRuleException {
		...
		searchMap.put("merchantId", merchantId);
		return warehouseProductWarnInfoMapper.getCountByWhere(searchMap);
	...
	public void deleteBy(Long merchantId,Long id) throws BizRuleException {
		...
		po = new WarehouseProductWarnInfo();
		po.setId(id);
	...
	public List<WarehouseProductWarnInfoDTO> getCacheWarehouseProductWarnInfoDTOList(Long merchantId) throws BizRuleException{
		if(merchantId==null || merchantId.longValue()<0) {
			logger.error("getCacheWarehouseProductWarnInfoDTOList merchantId is null");
		...
		return new CacheKeyLock(cachesKeyService, SysCacheTimeDMO.CACHETIMEOUT_30M) {//30分钟
			...
			protected Object doGetList(BaseCacheEnum type, String key) {
				...
				return BaseVo.copyListTo(retlist, WarehouseProductWarnInfoDTO.class);
			}
		}.getCache(CacheWarehouseEnum.WAREHOUSEPRODUCTWARNINFODTO_LIST,String.valueOf(merchantId), RedisCacheWaitTimeDMO.REDIS_TIMEOUT_1S);
